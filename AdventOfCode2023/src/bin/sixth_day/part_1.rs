...
fn main() {
    ...
    let mut distances = inputs.next().unwrap();
    let mut record = inputs.next().unwrap();

    let races = distances.map(|x| Race { length: x, record_distance: record.next().unwrap()});

    // assumes there will be at least 1 race with a result
    let mut result = 1;
    for (_index, input) in races.enumerate() {
        let possible = input.get_amount_possible();
        result *= possible;
    }
